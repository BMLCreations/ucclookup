...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-sm py-10">
      ...
      <p className="mt-1 text-sm text-slate-500">Welcome back to UCClookup.</p>

      <form action={loginAction} className="mt-6 space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        {error && (
      ...
      <p className="mt-4 text-center text-sm text-slate-500">
        No account? <Link href="/signup" className="font-medium text-indigo-700 hover:underline">Sign up</Link>
      </p>
    </div>
  );
}
